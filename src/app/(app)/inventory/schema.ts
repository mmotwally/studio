...
export const inventoryItemSchema = z.object({
  ...
  imageUrl: z.string().max(2048, { message: "Image path too long."}).optional().nullable(), // Only path, set server-side
  ...
  categoryId: z.string().min(1, { message: "Category is required." }), // Category is now required
  subCategoryId: z.string().optional(),
  locationId: z.string().optional(),
  supplierId: z.string().optional(),
  unitId: z.string().min(1, { message: "Unit of Measurement is required."}), // Unit of Measurement is now required
}).refine(data => {
